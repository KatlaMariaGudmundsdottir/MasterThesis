...
\subsection{Straight Line: Comparing the 3 Guidance Systems}
...
\subsubsection{Open-Loop Straight Line "following"}
...
\subsubsection{LOS Straight Line Following}
...
\subsubsection{ILOS Straight Line Following}
...
\subsection{Path 1: Comparing Open-Loop and Closed-Loop}
...
\subsubsection{Closed-Loop (LOS) Path-Following}
...
\subsection{Path 2: Comparing LOS and ILOS}
...
\end{figure}
Figure \ref{fig:ILOStwocurves} shows that the ILOS guidance system performs with higher precision an better convergence. The mean error is reduced to 0.11mm mostly due to the systems ability to eliminate the the steady-state error visible in the LOS system. The 2D path comparison shows a nearly perfect alignment between the actual and target paths after an initial period of convergence because of the initial offset.
...
We see in the results section on straight path performance that the Open loop performance is very poor as expected the lack of feedback leads to a serious drift even for a simple task as gaing down a straight path. This is largely due to the face that every ribbon has a certain pre-curvature, meaning the curvature the ribbon has when all tendon tensions are neutral (100mN) with such device inaccuracies and no feedback this inevtibaly leads to poor performance. Even though these pre-curvatures could be minimized in future iterations of the device manufacturing this would likely still be an issue since any small deviation from the expected curvature at the tip will accumulate over the distance of the path, since in gelatin a small curvature of the tip on entry will lead to a large total change in angle and position at the end of the insertion

We also see that in the curved path following where the open-loop system applies the feedforward actuation values determeined during the charactarization process that was done where a ribbon was inserted several times into the gelatin at constant tension actuation values. This was notably though done on a different ribbon. As discussed in the motivation on ribbon adaptation, there is a very significant difference in dynamics between the ribbons. This may have lead to the poor results seen in \ref{fig:curvedol} where the ribbon begins by curving too much towards the left and because of the lack of feedback this mistake then accumulates to the large error of 9mm. This shows that the feedback is necessary even though a thorough charactarization has been done on one ribbon.
...
\subsubsection{OpenLoop}
 The 2D path comparison illustrates significant deviation from the target path, especially toward the end. The system failed to correct for accumulated drift due to the lack of feedback, highlighting the limitations of open-loop operation in precision tasks.

pre-curvature

This continuous drift demonstrates the limitations of open-loop control in scenarios requiring precision, as there is no feedback to correct deviations.


\subsubsection{LOS Path-Following}

The tracking error remained relatively stable and low throughout the trajectory, with a minor increase near the end. The 2D path closely follows the desired straight line, indicating effective corrective behavior through LOS feedback, though some delay in response is evident.

 Although a slight increase in error occurs toward the end of the trajectory, the system is able to correct itself afterward, indicating effective use of feedback for error compensation.

 These results highlight the importance of feedback in achieving accurate and robust path-following in complex trajectories.

 suggesting that the system does not fully converge to the target path but instead follows a nearby offset trajectory. The 2D path comparison confirms that although the path shape is followed well, the ribbon consistently tracks slightly to the side of the intended path.
