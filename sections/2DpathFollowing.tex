...
\newline \newline
According to a review on flexible needles published in 2023 which surveys 131 articles \cite{lu_flexible_2023} the best reported performance is 0.3mm \cite{hong_magnetic_2021} in a brain phantom. For a comparison of the performance see figure  A very low error, but also over three times that of this system, proving the effectiveness of the novel ILOS-based approach which also has the benefit of necessitating much less exact modeling compared to these other systems. 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{per}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}


In a clinical context, stereotactic neurosurgical guidance systems, such as those used for deep brain stimulation electrode placement, typically report targeting accuracies between 0.6mm and 1.3mm \cite{zacest_precision_2009}, \cite{rajabian_accuracy_2022}, \cite{moser_measurement_2012}, which is generally accepted as sufficient for safe and effective procedures within the brain. Frame-based stereotaxy remains the gold standard with sub millimeter accuracy reported but rarely below 0.3-0.5mm in practice \cite{roth_accuracy_2018}.
\newline \newline
Although testing remains to be done in vitro, and more thorough randomized testing should be done in gelatin the 0.08mm accuracy indicates that the ILOS closed loop design and the ribbon device may have the potential to surpass existing clinical standards of accuracy.
